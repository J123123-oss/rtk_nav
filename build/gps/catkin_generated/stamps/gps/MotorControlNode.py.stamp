...
RTK_WAYPOINT_TOLERANCE = 0.2         # 到达目标点的距离 tolerance（米）
IMU_YAW_OFFSET = 0.0                 # IMU偏航角校准值（根据实际安装调整）
LINEAR_SPEED_BASE = 0.2              # 基础线速度（m/s，需根据电机减速比/轮径转换）
...
# 主机ID与通信类型（手册私有协议）
MOTOR_MASTER_ID = 253      # 主机ID（0-127，手册限制，需与硬件匹配）
...
class RTKNavigator:
    def __init__(self):
        ...
        self.current_gps: Tuple[float, float] = (0.0, 0.0)  # 当前GPS位置
    ...
    def imu_callback(self, msg: Imu) -> None:
        ...
        # 四元数转偏航角（参考ROS sensor_msgs/Imu格式）
        x, y, z, w = msg.orientation.x, msg.orientation.y, msg.orientation.z, msg.orientation.w
        self.imu_yaw = math.atan2(2 * (w * z + x * y), 1 - 2 * (y * y + z * z)) - IMU_YAW_OFFSET
        self.imu_yaw = math.fmod(self.imu_yaw + math.pi, 2 * math.pi) - math.pi  # 归一化到[-π, π]
...
class MotorControlNode:
    def __init__(self):
        ...
        rospy.Subscriber("/gps/current_position", Point, self.rtk_nav.gps_callback)  # GPS位置
        rospy.Subscriber("/imu/data", Imu, self.rtk_nav.imu_callback)  # IMU数据
    ...
    def rad_from_linear(self, linear_speed: float) -> float:
        ...
        # 公式：rad/s = (linear_speed * 减速比) / 轮径（单位：m）
        # 示例：减速比7.75，轮径0.1m → rad/s = (v *7.75)/0.1 = 77.5*v
        reduction_ratio = 7.75  # 电机减速比（手册0x201f参数）
        wheel_diameter = 0.1     # 车轮直径（m，需根据实际硬件调整）
    ...
    def set_motors_speed(self, left_speed: float, right_speed: float) -> None:
        ...
        # 右电机（ID128，注意：手册CAN_ID最大127，建议改为≤127）
        self.motor_ctrl.motor_set_speed(128, right_speed)
        # 发布速度消息
        speed_msg = Twist()
