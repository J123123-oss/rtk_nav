...
class IMUParser:
    ...
    def publish_inspvae_data(self, angles):
        ...
        msg.header.stamp = rospy.Time.now()  # 或 angles.header.stamp（如果输入有时间戳）
        msg.header.frame_id = "base_link"   # 根据实际坐标系填写（如"imu_link"）
        msg.vector.z = angles.data % 360    # 通常用z轴表示yaw（航向角）
